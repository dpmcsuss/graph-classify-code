...
alg.s0_trn = [2:2:18 18 18 18 18];      % list of #'s of hold-out samples for class 0
alg.s1_trn = 2:2:26;                    % list of #'s of hold-out samples for class 1
...
xval.num_repeats=alg.num_repeats;
for i=1:length(alg.s0_trn)
    xval.s0_trn=alg.s0_trn(i);
    xval.s1_trn=alg.s1_trn(i);
    
    Lhat(i) = classify_holdout_par(As,constants,xval,subspace);
...
for i=1:alg.trn_s
    for j=1:alg.num_repeats
        Lavg_tmp(i).(fn{k})(j)=Lhat(i).plugin(j).(fn{k});
