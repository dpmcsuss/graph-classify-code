...
rm(list=ls())
Ac=read.delim(file("~/Research/necog/BLSA1/data/john/MeanFA.csv"),header=FALSE,sep="")
n = length(Ac)
Ac=as.matrix(Ac,nrow=n,ncol=n)
Ac=matrix(Ac,nrow=n,ncol=n)
Ac=Ac+t(Ac)
NaNind=is.na(Ac)
Ac[NaNind]=0
Ac=Ac*10
...
print(paste("Lhat-coherentsubspace",Lhat[z]))
 }
