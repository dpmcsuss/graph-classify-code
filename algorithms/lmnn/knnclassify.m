function [Eval,Details]=knnclassify(L,xTr,yTr,xTe,yTe,KK)
...
yTr2=zeros(length(KK),NTr);
yTe2=zeros(length(KK),NTe);
...
for i=1:B:max(NTr,NTe)
    if(i<=NTr)
        ...
        nn=nn(2:Kn+1,:);
        yTr2(:,i:i+BTr)=LSKnn2(yTr(nn),KK,MM);
        iTr(:,i:i+BTr)=nn;
        Eval(1,:)=sum((yTr2(:,1:i+BTr)~=repmat(yTr(1:i+BTr),length(KK),1))',1)./(i+BTr);
    end;
    if(i<=NTe)
        BTe=min(B-1,NTe-i);
        Dtr=addh(addv(-2*xTr'*xTe(:,i:i+BTe),sx1),sx2(i:i+BTe));
        [dist,nn]=mink(Dtr,Kn);
        yTe2(:,i:i+BTe)=LSKnn2(reshape(yTr(nn),max(KK),BTe+1),KK,MM);
        iTe(:,i:i+BTe)=nn;
        Eval(2,:)=sum((yTe2(:,1:i+BTe)~=repmat(yTe(1:i+BTe),length(KK),1))',1)./(i+BTe);
    end;
